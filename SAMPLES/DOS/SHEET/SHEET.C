...
void InitialiseSheetModel(br_model *model) {
    ...
    BrVector3Set(&vertices[(SHEETY-1)*SHEETX].p,
	BR_SCALAR(-0.5),BR_SCALAR(-0.5),BR_SCALAR(-0.5));

    BrVector3Set(&vertices[SHEETY-1+(SHEETY-1)*SHEETX].p,
	BR_SCALAR(-0.5),BR_SCALAR(-0.5),BR_SCALAR(-0.5));
...
int AccumulatePlaneField(br_vector3 *a,br_vector3 *pos,br_vector3 *normal,
	br_scalar k) {
    br_vector3 tmp1,tmp2;
    br_scalar length;
...
void
AccumulateForceFromSpring(br_vector3 *force,int i,int j,int d,int r) {
    if (r>0) {
...
void UpdateDynamics(br_model *model,br_vector3 *gravity) {
	...
	for (i = 0; i<SHEETX; i++) {
	    ...
	    if (
		AccumulateSphereField(&acceleration,&VERTEX(model,i,j).p,&centre1) ||
...
void PlaceSphere(br_actor *parent,br_model *model,br_vector3 *position,char *colour) {
    ...
    BrMatrix34PostTranslate(&sphere->t.t.mat,
...
main() {
    ...
    */
    ...
    BrMatrix34PostTranslate(&world->t.t.mat,
        0,BR_SCALAR(0.5),0);
    ...
    sheet->model = BrModelAllocate("sheet",
		SHEETX*SHEETY,4*(SHEETX-1)*(SHEETY-1));
    InitialiseSheetModel(sheet->model);
    sheet->model->flags = BR_MODF_UPDATEABLE;
    ...
    BrMatrix34PostScale(&box->t.t.mat,
        BR_SCALAR(1.0),BR_SCALAR(1.0),BR_SCALAR(1.0));
	...
	BR_SCALAR(0.0),BR_SCALAR(0.0),BR_SCALAR(4.0));
    ...
    BrMatrix34PostTranslate(&light2->t.t.mat,
	BR_SCALAR(0),BR_SCALAR(1),BR_SCALAR(1));
	...
	if (!Frozen) {
	    ...
	    BrModelUpdate(sheet->model,BR_MODU_VERTEX_POSITIONS);
	}

	/* Allow user to move the system around */

	mouse_x = mouse_y = 0;
	DOSMouseRead((br_int_32 *)&mouse_x,(br_int_32 *)&mouse_y,(br_uint_32 *)&mouse_buttons);
	if (mouse_x || mouse_y) {
	    if (mouse_buttons & BR_MSM_BUTTONL) {
		BrMatrix34PostTranslate(&pivot->t.t.mat,
			BR_MUL(BR_SCALAR(mouse_x),BR_SCALAR(0.01)),
			BR_MUL(BR_SCALAR(-mouse_y),BR_SCALAR(0.01)),0);
	    } else if (mouse_buttons & BR_MSM_BUTTONR) {
		BrMatrix34PostTranslate(&pivot->t.t.mat,
			BR_MUL(BR_SCALAR(mouse_x),BR_SCALAR(0.01)),0,
			BR_MUL(BR_SCALAR(-mouse_y),BR_SCALAR(0.01)));
	    } else {
		BrMatrix34RollingBall(&rotation,mouse_x,-mouse_y,800);
		BrMatrix34Post(&system->t.t.mat,&rotation);
		BrMatrix34Inverse(&inverse,&system->t.t.mat);
		BrMatrix34ApplyV(&gravity,&init_gravity,&inverse);
	    }
	}
